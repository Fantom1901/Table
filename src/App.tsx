...
const hotelColumns = [
  hotelHelper.accessor("name", { header: () => <span className="flex items-center gap-2"><Hotel size={16}/> Название</span> }),
  hotelHelper.accessor("address", { header: () => <span className="flex items-center gap-2"><MapPin size={16}/> Адрес</span> }),
  hotelHelper.accessor("roomType", { header: () => <span className="flex items-center gap-2"><BedDouble size={16}/> Тип номера</span> }),
...
type CustomRow = { col1: string; col2: string; col3: string; col4: string; col5: string; colSpanId?: "row1" | "row4"; };
const customData: CustomRow[] = [
  { col1: "Панель управления", col2: "", col3: "", col4: "", col5: "", colSpanId: "row1" },
        ...
        <div className="bg-white/70 backdrop-blur-md p-6 rounded-3xl shadow-sm border border-slate-100">
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200">
            <table className="w-full border-collapse text-left text-sm text-slate-600">
              <thead className="bg-slate-50/75 text-slate-700 border-b border-slate-200">
                {hotelTable.getHeaderGroups().map(hg => (
                  <tr key={hg.id}>
                    {hg.headers.map(h => <th key={h.id} className="p-4 font-semibold text-slate-900 border-b border-slate-200">{flexRender(h.column.columnDef.header, h.getContext())}</th>)}
                  </tr>
                ))}
              </thead>
              <tbody>
                {hotelTable.getRowModel().rows.map((row, rIdx) => (
                  <tr key={row.id} className="border-b border-slate-200 hover:bg-slate-50/50 transition-colors">
                    {row.getVisibleCells().map(cell => {
                      ...
                      // Логика вертикального объединения (rowSpan)
                      if (id === "name") {
                        if (orig.rowSpanName) {
                          return <td key={cell.id} rowSpan={orig.rowSpanName} className="p-4 font-medium text-slate-900 bg-slate-50/50 align-top border-r border-slate-200">{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>;
                        }
                        return <td key={cell.id} className="hidden" />;
                      }
                      if (id === "address") {
                        if (orig.rowSpanAddress) {
                          return <td key={cell.id} rowSpan={orig.rowSpanAddress} className="p-4 text-slate-500 align-top border-r border-slate-200">{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>;
                        }
                        return <td key={cell.id} className="hidden" />;
                      }

                      return <td key={cell.id} className={`p-4 border-r border-slate-200 last:border-r-0 ${id === "price" ? "font-mono text-emerald-600 text-center" : ""}`}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>;
                    })}
                  </tr>
                ))}
              </tbody>
            ...
            <table className="w-full border-collapse text-left text-sm text-slate-600">
              <thead className="bg-slate-800 text-white">
                {customTable.getHeaderGroups().map(hg => (
                  <tr key={hg.id}>
                    {hg.headers.map(h => <th key={h.id} className="p-4 font-medium border-b border-slate-900">{flexRender(h.column.columnDef.header, h.getContext())}</th>)}
                  </tr>
                ))}
              </thead>
              <tbody>
                {customTable.getRowModel().rows.map(row => {
                  const orig = row.original;
                  return (
                    <tr key={row.id} className={`border-b border-slate-200 transition-colors ${orig.col5 === "Warning" ? "bg-amber-50/60" : ""}`}>
                      {row.getVisibleCells().map(cell => {
                        const id = cell.column.id;

                        // Строка 1: Полное объединение на 5 колонок
                        if (orig.colSpanId === "row1") {
                          if (id === "col1") {
                            return <td key={cell.id} colSpan={5} className="p-4 font-semibold text-center bg-indigo-50/80 text-indigo-900 border-r-0">⚡ {orig.col1}</td>;
                          }
                          return <td key={cell.id} className="hidden" />;
                        }

                        // Строка 4: Объединение 4 колонок текста + 5-я колонка статуса отдельно
                        if (orig.colSpanId === "row4") {
                          if (id === "col1") {
                            return <td key={cell.id} colSpan={4} className="p-4 font-medium italic text-slate-500 bg-slate-50/80 border-r border-slate-200">{orig.col1}</td>;
                          }
                          if (id === "col5") {
                            return <td key={cell.id} className="p-4 text-slate-500 font-medium bg-slate-50/80 text-center">{orig.col5}</td>;
                          }
                          return <td key={cell.id} className="hidden" />;
                        }
                        ...
                        return (
                          <td key={cell.id} className={`p-4 border-r border-slate-200 last:border-r-0 ${id === "col5" ? orig.col5 === "Online" ? "text-emerald-600 font-semibold" : orig.col5 === "Warning" ? "text-amber-600 font-bold" : "text-slate-400" : ""}`}>
        ...
        <div className="bg-white/70 backdrop-blur-md p-6 rounded-3xl shadow-sm border border-slate-100">
          ...
          <div className="space-y-4">
            {[
              ...
              { q: "Какими атрибутами обладает тег <table>?", a: "В старом HTML4/XHTML: border, width, align, bgcolor, bordercolor, cellspacing, cellpadding. В современном веб-стандарте все они заменены на CSS-свойства." },
              ...
              { q: "Как выровнять содержимое ячеек в HTML-документе?", a: "Раньше использовались атрибуты align (по горизонтали) и valign (по вертикали). Сейчас это делается через CSS: text-align и vertical-align." }
            ].map((item, idx) => (
              <div key={idx} className="p-4 bg-slate-50 rounded-2xl border border-slate-200/50">
